...
Start the container
```bash
docker run --init -d --rm -p 9050:9050 -v /tmp/local-bigquery/:/data --name bigquery ghcr.io/novucs/local-bigquery:0.2.1
```
...
services:
  bigquery:
    image: ghcr.io/novucs/local-bigquery:0.2.1
    ports:
      - "9050:9050"
    environment:
...
bigquery_image = "ghcr.io/novucs/local-bigquery:0.2.1"
