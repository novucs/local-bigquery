...
CREATE MACRO array_first(a) AS
    CASE WHEN len(a) = 0 THEN error('ARRAY_FIRST cannot get the first element of an empty array') ELSE a[1] END;

CREATE MACRO array_last(a) AS
    CASE WHEN len(a) = 0 THEN error('ARRAY_LAST cannot get the last element of an empty array') ELSE a[-1] END;
...
CREATE MACRO generate_array(first, last) AS list_transform(range(0, last - first + 1), i -> first + i),
    (first, last, step) AS CASE
        WHEN step = 0 THEN error('Sequence step cannot be 0.')
...
CREATE MACRO _array_at(a, i, base) AS CASE
    ...
    WHEN i - base < 0 THEN error('Array index ' || i || ' is out of bounds (underflow)')
    WHEN i - base >= len(a) THEN error('Array index ' || i || ' is out of bounds (overflow)')
    ELSE a[i - base + 1]
END;
