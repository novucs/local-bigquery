...
CASES = [
    ...
    q("SELECT 2147483647 + 1", 2147483648, xfail="integer literals typed as INT32"),
    ...
    q(
        ...
        xfail="division by zero returns NULL",
    ),
    q(
        "SELECT DIV(1, 0)",
        error="division by zero",
    ...
    q("SELECT IEEE_DIVIDE(0, 0)", math.nan, xfail="missing function"),
    ...
    q(
        f"SELECT -({INT64_MIN})",
        error="(?i)overflow",
        xfail="negating INT64 min does not error",
    ),
    q(f"SELECT ABS({INT64_MIN})", error="(?i)overflow"),
    q(f"SELECT SAFE_ADD({INT64_MAX}, 1)", None, xfail="missing function"),
    q(f"SELECT SAFE_SUBTRACT({INT64_MIN}, 1)", None, xfail="missing function"),
    q(f"SELECT SAFE_MULTIPLY({INT64_MAX}, 2)", None, xfail="missing function"),
    q(f"SELECT SAFE_NEGATE({INT64_MIN})", None, xfail="missing function"),
    q(
    ...
    q(
        ...
        rows=[(-1, 0, 1.0)],
        xfail="SIGN of FLOAT64 returns INT64",
    ),
    ...
    q(
        ...
        xfail="NaN sorts after numbers",
    ),
    q("SELECT 5 & 3, 5 | 3, 5 ^ 3, ~0", rows=[(1, 7, 6, -1)]),
    q("SELECT 1 << 4, 1 << 64", rows=[(16, 0)], xfail="shift past 63 bits errors"),
    q("SELECT -16 >> 2", 4611686018427387900, xfail="right shift sign-extends"),
    q("SELECT BIT_COUNT(7)", 3),
    q(r"SELECT b'\x05' & b'\x03'", b"\x01", xfail="bitwise ops on BYTES unsupported"),
    q("SELECT RAND() >= 0 AND RAND() < 1", True),
    q("SELECT RANGE_BUCKET(20, [0, 10, 20, 30])", 3, xfail="missing function"),
    q("SELECT RANGE_BUCKET(-1, [0, 10])", 0, xfail="missing function"),
    q("SELECT RANGE_BUCKET(5, [])", 0, xfail="missing function"),
    q("SELECT RANGE_BUCKET(NULL, [1])", None, xfail="missing function"),
